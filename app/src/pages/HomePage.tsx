...
export function HomePage() {
  return <><a className="skip-link" href="#main-content">本文へ移動</a><SiteHeader /><main id="main-content">
    <section id="top" className="hero"><div className="container hero-grid">
      <div className="hero-copy"><p className="eyebrow"><a className="hackathon-link" href="https://odhackathon.metro.tokyo.lg.jp/" target="_blank" rel="noreferrer">東京都知事杯オープンデータ・ハッカソン2026 参加チーム <span aria-hidden="true">↗</span></a></p><h1><span>Civic</span><em>Unknot</em></h1><p className="catchphrase">絡まりは、ほどける。<br />気づきは、つながる。</p><p className="mission">制度や支援情報が複雑に見えるとき、一人で抱え込まないための入口をつくります。CivicUnknotは、東京都のオープンデータを使い、気づきを信頼できる公的な相談窓口へつなぐチームです。</p><div className="button-row"><a className="button button-primary" href={demoUrl} target="_blank" rel="noreferrer">trait-compass を試す <span aria-hidden="true">↗</span></a><a className="button button-secondary" href="#product">できることを見る</a></div><p className="cta-reassurance">登録不要・ブラウザで完結・医療診断ではありません</p></div>
    ...
    <section id="product" className="section section-tint"><div className="container"><div className="section-heading"><div className="product-lockup"><img src="./trait-compass-icon.svg" alt="trait-compass のコンパスアイコン" /><div><p className="section-kicker">PRODUCT / TRAIT-COMPASS</p><h2>「整理する」と「探す」を、<br />ひとつの流れに。</h2></div></div><p className="lead"><strong>trait-compass</strong> は、発達特性と日常の困りごとを整理し、公的な支援情報へつなぐブラウザ完結型のセルフチェックアプリです。医療行為ではありません。</p></div>
      <div className="entry-options" aria-label="trait-compassの始め方"><a className="entry-option" href={demoUrl} target="_blank" rel="noreferrer"><span className="entry-icon" aria-hidden="true">◎</span><span><strong>セルフチェックから</strong><small>約5分・30問で傾向を整理する</small></span><b aria-hidden="true">→</b></a><a className="entry-option" href={demoUrl} target="_blank" rel="noreferrer"><span className="entry-icon" aria-hidden="true">⌕</span><span><strong>相談先を探すことから</strong><small>年齢と地域から公的な窓口を探す</small></span><b aria-hidden="true">→</b></a></div>
  ...
  </main><a className="mobile-cta" href={demoUrl} target="_blank" rel="noreferrer">セルフチェックを始める <span aria-hidden="true">↗</span></a><footer><div className="container footer-content"><div className="footer-brand"><img src="./logo-transparent-outline.png" alt="CivicUnknot チームロゴ" /><p>© 2026 CivicUnknot</p></div><div><p>東京都知事杯オープンデータ・ハッカソン2026 参加チーム</p><p>支援情報データ出典: 東京都オープンデータカタログサイトおよび区市町村オープンデータ</p></div></div></footer>
  </>
}
